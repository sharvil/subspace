...
namespace Database
{
	class BanLift: public DbAction
	{
		public:
			explicit BanLift(uint32 sysopId, uint32 zoneId, uint32 banId)
				: iSysopId(sysopId),
				  iZoneId(zoneId),
				  iBanId(banId)
			{ }
			...
			void operator()(Transaction & t)
			{
				...
				if(access > targetAccess || (isNetBan && !canNetBan))
				{
					...
					if(access > targetAccess || (isNetBan && !canNetBan))
						throw permission_error("cannot lift ban");
				}

				query = Format("delete from isometry.ban where id=%d", iBanId);
				t.exec(query);
			}
